...
double CircleMode::GetVelocityY()
{
  return m_velocityY;
}
...
{
  m_omega1 = (((1 / s_RW) * (  (-m_velocityY * s_sqrt3of2) + ((2 * s_PI * s_L) / inTime) / 3 )) * s_PRM);
}
/*
...
{
  m_omega2 = ((((2 * s_PI * s_L) / (inTime)/ 3 ) / s_RW) * s_PRM);
}
...
{
  m_omega3 = (((1 / s_RW) * (  (m_velocityY * s_sqrt3of2) + ((2 * s_PI * s_L) / inTime) /3 )) * s_PRM);
}
